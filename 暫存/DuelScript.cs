...
public class DuelScript : LeadToSurviveGameBaseClass
{
    ...
    public void HeroData(HeroScript _playerHeroScript, HeroScript _enemyHeroScript)
    {
        ...
        Vector2 _playerPos = PlayerHero._Tf.position;
        _playerPos.y = 0;
        CountdownText.transform.position = _playerPos;

        EnemyHero = _enemyHeroScript;
        EnemyHero.IsItPossibleToDuel = true;
    ...
    public void PlayerMoveClick(HeroState _hState)
    {
        ...
        // 當動作清單數量等於 MoveListMax 時，表示動作已經選擇完成
        if (PlayerMoveList.Count == MoveListMax)
        {
    ...
    public void HerosDistance()
    {
        ...
        { //開始動作
            ...
            if (_distance < 1.5f)
            {
                ...
                IsItPossibleToformalDuel = false;
                HeroRun_DashOrMiss(PlayerHero, PlayerHeroTf, EnemyHeroTf, false, false);
                HeroRun_DashOrMiss(EnemyHero, EnemyHeroTf, PlayerHeroTf, false, false);
                return;
            }
        ...
        {
            HeroRun_DashOrMiss(PlayerHero, PlayerHeroTf, EnemyHeroTf, false); // 走路或衝刺
            HeroRun_DashOrMiss(EnemyHero, EnemyHeroTf, PlayerHeroTf, false); // 走路或衝刺
        }
        else if (_distance > 5) // 距離不算遠
        {
            // 執行走路
            HeroRun_DashOrMiss(PlayerHero, PlayerHeroTf, EnemyHeroTf, true);
            HeroRun_DashOrMiss(EnemyHero, EnemyHeroTf, PlayerHeroTf, true);
            IsItPossibleToformalDuel = false;
        }
    }
    /// <summary>
    ...
    /// <param name="reverse">是否強制為反向</param>
    private void HeroRun_DashOrMiss(HeroScript _heroScript, Transform _tf, Transform _EnemyTf, bool isRun = true, bool reverse = false)
    {
        ...
        //_heroScript.HeroDuelStateFunc(HeroState.Run, _Direction);
        //return;
        ...
        if (IsMiss) // 有閃現
        {
            GameManagerScript.DuelHeroMiss(_heroScript, Timetime);
        }
        else if (IsDash) // 有衝刺
        {
            GameManagerScript.DuelHeroDash(_heroScript, Timetime);
        }
        else
        {
            _heroScript.HeroDuelStateFunc(HeroState.Run, _Direction);
    ...
    public void FormalDuel()
    {
        ...
        {
            bugTime = 0; //計算bug時間歸零
            if (EnemyMoveList.Count == MoveIndex || PlayerHero.Hp <= 0 || EnemyHero.Hp <= 0) //動作清單索引已經跟數量一樣
            {
            ...
            IsItPossibleToformalDuel = RandomHeroState();
            if (!IsItPossibleToformalDuel)
            {
                if (!RandomHeroState())
                {
                    HeroRun_DashOrMiss(PlayerHero, PlayerHeroTf, EnemyHeroTf, false, true); //玩家移動
                }
                if (!RandomHeroState())
                {
                    HeroRun_DashOrMiss(EnemyHero, EnemyHeroTf, PlayerHeroTf, false, true); //敵人移動
                }
                //HeroRun_DashOrMiss(PlayerHero, PlayerHeroTf, EnemyHeroTf, false, true); //玩家移動
                //HeroRun_DashOrMiss(EnemyHero, EnemyHeroTf, PlayerHeroTf, false, true); //敵人移動
                return;
            ...
            float distance = Vector2.Distance(PlayerHeroTf.position, EnemyHeroTf.position);
            PlayerAnimator.speed = 2f; //玩家動畫速度變1.5倍
            HeroState pState = PlayerMoveList[MoveIndex];
            switch (pState)
            {
                case HeroState.Attack:
                    //if (distance <= PlayerHero.phySize) PlayerHitTotal += PlayerHero.OffsetValue(PlayerDataObject.Percentage, PlayerHero.Attack, PlayerDataObject.PlayerHeroLv);
                    PlayerHero.HeroDuelStateFunc(pState, PlayerHero.enemyCollider, EnemyHero, false);
                    break;
                case HeroState.Def:
            ...
            EnemyAnimator.speed = 2f; //敵人動畫速度變1.5倍
            HeroState eState = EnemyMoveList[MoveIndex];
            switch (eState)
            {
                case HeroState.Attack:
                    //if (distance <= EnemyHero.phySize) EnemyHitTotal += EnemyHero.OffsetValue(EnemyDataObject.Percentage, EnemyHero.Attack, EnemyDataObject.PlayerHeroLv);
                    EnemyHero.HeroDuelStateFunc(eState, EnemyHero.enemyCollider, PlayerHero, false);
                    break;
                case HeroState.Def:
                    PlayerHero.HeroDuelStateFunc(eState);
                    break;
            }
        ...
        if (playerNormalizedTime >= .9f) PlayerMoveIsOK = true; //玩家動作完成

        float enemyNormalizedTime = EnemyHero.AnimatorStateInfoNormalizedTime(); //敵人動畫正規化時間
        if (enemyNormalizedTime >= .9f) EnemyMoveIsOK = true; //敵人動作完成
    ...
    IEnumerator RetreatOrDie(HeroScript hero, HeroScript enemy, Transform _Tf, Transform _enemyTf)
    {
        ...
        for (int i = 0; i < 20; i++)
        {
            ...
            if (hero.Hp <= 0)
                //玩家血量小於等於0，玩家位置往後飛越
                _Tf.position = Vector3.Slerp(_Tf.position, _DirectionPlayer, 0.2f);
            else
                //玩家血量大於0，玩家位置往後移動
                _Tf.position = Vector2.MoveTowards(_Tf.position, _DirectionPlayer, 0.2f);

            if (enemy.Hp <= 0)
                //敵人血量小於等於0，敵人位置往後飛越
                _enemyTf.position = Vector3.Slerp(_enemyTf.position, _DirectionEnemy, 0.2f);
            else
                //敵人血量大於0，敵人位置往後移動
                _enemyTf.position = Vector2.MoveTowards(_enemyTf.position, _DirectionEnemy, 0.2f);
